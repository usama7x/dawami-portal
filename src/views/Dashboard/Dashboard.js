...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getDashboardData().then(async res=>{
      ...
      res = res.data;
      // res = dashboardData;
      let pcheckedInUser = res.checkedInUsers.map( async (currentValue, index)=>{
        return [
          currentValue.userImagePath?await getImage(currentValue.userImagePath):'',
          currentValue.userName,
          currentValue.checkInTime
        ]
      });
      let punCheckedInUser = res.unCheckedInUsers.map(async (currentValue, index)=>{
        return [
          currentValue.userImagePath?await getImage(currentValue.userImagePath):'',
          currentValue.userName
        ]
        ...
        <Card>
          ...
          <Card>
            <CardHeader color="warning">
              <h4 className={classes.cardTitleWhite}>Checked-in Employees Count</h4>
