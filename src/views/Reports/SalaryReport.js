...
import { getOrgUsersSalaryApi, getOrgUsersSalaryExcelApi, getAttendanceApi } from '../../utils/api.js'
...
const SalaryReport = memo(() => {
    ...
    let defaultTime = {
        startTime: (new Date(currentDate.setDate(currentDate.getDate() - 7))),
        endTime: (new Date())
    }
    let timeIso = {
    ...
    const onSearch = (data) => {
        ...
        if (empId) {
            ...
            getOrgUsersSalaryApi(timeIso.startTime, timeIso.endTime).then(res => {
                updateList(res.data);
                setLoader(false)
            })
        }
                ...
                <Table className={classes.table} aria-label="customized table" >
                    <TableHead>
                        <TableRow>
                            ...
                            <StyledTableCell align="right">Rebate</StyledTableCell>
                            <StyledTableCell align="right">Net (Salary - Rebate)</StyledTableCell>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {empData.map((row) => (
        ...
        return (
            <TableContainer component={Paper}>
                <Table className={classes.table} aria-label="customized table">
                    <TableHead>
                        <TableRow>
                            <StyledTableCell align="right">attendanceId</StyledTableCell>
                            ...
                            <StyledTableCell align="right">Early Checkin Minutes</StyledTableCell>
        ...
        <div>
            <GridContainer>
                ...
                <GridItem xs={12} sm={12} md={6}>
                    <SearchForm dateOnly={true} onSubmit={onSearch} validation={true} startDate={defaultTime.startTime} endDate={defaultTime.endTime} />
                </GridItem>
                <GridItem xs={12} sm={12} md={6}>
                    <div style={{ float: "right" }}>
                        {
                            ...
                            ) && <DownloadReport startDate={defaultDate.startTime} userId={empId ? empId.empId : null} endDate={defaultDate.endTime} reportApi={getOrgUsersSalaryExcelApi} />
                        }
                        {
                            (
                                (!empId && empData && empData.length != 0) ||
                            ...
                            <PrintReport elementRef={reportRef.current}  />
                        }
                    </div>
                </GridItem>
            </GridContainer>
